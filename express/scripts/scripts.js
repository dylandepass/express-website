...
export function getMeta(name) {
    ...
    const $metas = [...document.querySelectorAll('meta')].filter(($m) => {
        ...
        return (
            (nameAttr && nameLower === nameAttr.toLowerCase()) ||
...
function resolveFragments() {
    Array.from(document.querySelectorAll('main > div div'))
        ...
        .filter(($cell) => /^\[[A-Za-z0-9 -_—]+\]$/gm.test($cell.textContent))
        .forEach(($cell) => {
            ...
            const $marker = Array.from(
                document.querySelectorAll('main > div h3')
            ).find(
                ($title) => $title.textContent.toLocaleLowerCase() === marker
            );
            if (!$marker) {
            ...
            setTimeout(() => {
                $cell.innerHTML = '';
                Array.from($fragment.children).forEach(($elem) =>
...
function decorateMarqueeColumns($main) {
    ...
    const $firstColumnsBlock = $main.querySelector(
        '.section-wrapper:first-of-type .columns:first-of-type'
    );
    ...
    if (
        ...
        try {
            const cssLoaded = new Promise((resolve) => {
                loadCSS(
                    `/express/blocks/${blockName}/${blockName}.css`,
                    resolve
                );
            });
                ...
                (async () => {
                    try {
                        const mod = await import(
                            `/express/blocks/${blockName}/${blockName}.js`
                        );
                        if (mod.default) {
                            await mod.default(
                                block,
                        ...
                        console.log(
                            `failed to load module for ${blockName}`,
                            err
                        );
                    }
                    resolve();
...
export function loadBlocks($main) {
    const blockPromises = [
        ...$main.querySelectorAll('div.section-wrapper > div > .block')
    ...
    if (!window.placeholders) {
        try {
            ...
            json.data.forEach((placeholder) => {
                window.placeholders[toClassName(placeholder.Key)] =
                    placeholder.Text;
            });
        } catch {
...
function addPromotion() {
    ...
    if (!document.querySelector('main .promotion')) {
        ...
        if (category) {
            ...
            if (promos[category]) {
                const $promoSection = createTag('div', {
...
function loadMartech() {
    ...
    if (
        !(
            martech === 'off' ||
            document.querySelector(`head script[src="${analyticsUrl}"]`)
        )
    ) {
...
function loadGnav() {
    ...
    if (
        !(
            gnav === 'off' ||
            document.querySelector(`head script[src="${gnavUrl}"]`)
        )
    ) {
...
export function addSearchQueryToHref(href) {
    const isCreateSeoPage =
        window.location.pathname.includes('/express/create/');
    const isDiscoverSeoPage =
        window.location.pathname.includes('/express/discover/');
    const isPostEditorLink = postEditorLinksAllowList.some((editorLink) =>
        href.includes(editorLink)
    );
...
export function decorateButtons(block = document) {
    ...
    block.querySelectorAll(':scope a').forEach(($a) => {
        ...
        if (
            ...
            if (
                $a.textContent.trim().startsWith('{{icon-') &&
                $a.textContent.trim().endsWith('}}')
            ) {
                const $iconName = /{{icon-([\w-]+)}}/g.exec(
                    $a.textContent.trim()
                )[1];
                if ($iconName) {
...
async function decorateTesting() {
    ...
    const martech = usp.get('martech');
    if (
        (checkTesting() && martech !== 'off' && martech !== 'delay') ||
        martech === 'rush'
    ) {
        // eslint-disable-next-line no-console
...
export async function fixIcons(block = document) {
    ...
    block.querySelectorAll('svg use[href^="./_icons_"]').forEach(($use) => {
        $use.setAttribute(
            'href',
            `/express/icons.svg#${$use.getAttribute('href').split('#')[1]}`
        );
    });
        ...
        if (alt) {
            ...
            if (lowerAlt.includes('icon:')) {
                ...
                $picture.parentElement.replaceChild(
                    getIconElement([icon, mobileIcon], size, altText),
                    $picture
                );
            }
        }
...
function makeRelativeLinks($main) {
    $main.querySelectorAll('a').forEach(($a) => {
        if (!$a.href) return;
        try {
            const { protocol, hostname, pathname, search, hash } = new URL(
                $a.href
            );
            if (
                ...
                hostname !== 'adobesparkpost.app.link' &&
...
function displayOldLinkWarning() {
    if (
        ...
        document
            .querySelectorAll('main a[href^="https://spark.adobe.com/"]')
            .forEach(($a) => {
                const url = new URL($a.href);
    ...
    try {
        ...
        if (document.referrer) {
            ...
            const expressEnvs = [
                ...
                'express-qa.adobe.com',
                'express-dev.adobe.com'
            ];
            if (
                url.hostname.endsWith('.adobeprojectm.com') ||
                expressEnvs.includes(url.hostname)
            ) {
...
function generateFixedButton() {
    if (document.body.classList.contains('has-fixed-button')) {
        ...
        const hideFixedButtonWhenInView = new IntersectionObserver(
            (entries) => {
...
async function wordBreakJapanese() {
    ...
    const { loadDefaultJapaneseParser } = await import(
        './budoux-index-ja.min.js'
    );
    const parser = loadDefaultJapaneseParser();
    document
        .querySelectorAll('h1, h2, h3, h4, h5, p:not(.button-container)')
        .forEach((el) => {
...
function stamp(message) {
    if (window.name.includes('performance')) {
        // eslint-disable-next-line no-console
        console.log(
            `${new Date() - performance.timing.navigationStart}:${message}`
        );
    }
}
    ...
    try {
        ...
        const pores = new PerformanceObserver((entryList) => {
            ...
            entries.forEach((entry) => {
                stamp(
                    `resource loaded: ${entry.name} - [${Math.round(
